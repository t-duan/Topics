...
def upload_file():
    ...
    doc_term_matrix = preprocessing.create_doc_term_matrix(corpus, corpus.index)
    ...
    if getattr(sys, 'frozen', False):
        doc_topic_plot.savefig(os.path.join(sys._MEIPASS, 'static', 'topics.png'))
    else:
        doc_topic_plot.savefig(os.path.join('static', 'topics.png'))
    doc_topic_plot.close()
    print("Rendering result.html ...")
    return render_template('result.html', topics=[topics.to_html(classes='df')], doc_topics=[doc_topics.iloc[:5].T.iloc[:2].T.to_html(classes='df')])
